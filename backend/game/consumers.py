    ...
    async def receive_json(self, content, **kwargs):
        ...
        if msg == "pick.set" and sender_role in ("p1", "p2"):
            ...
            if state.get("phase") != "minus":
                ...
                if idx in (0, 1):
                    if ready["minus"].get(sender_role, False):
                        pass
                    else:
                        if sender_role == "p2" and not ready["minus"]["p1"]:
                            pass
                        else:
                            state[f"{sender_role}Removed"] = idx
                            ready["minus"][sender_role] = False

        elif msg == "phase.reveal" and sender_role in ("p1", "p2"):
            if state.get("phase") == "minus":
                if sender_role == "p1":
                    if state.get("p1Removed") is not None:
                        ready["minus"]["p1"] = True
                elif sender_role == "p2":
                    if ready["minus"]["p1"] and state.get("p2Removed") is not None:
                        ready["minus"]["p2"] = True
